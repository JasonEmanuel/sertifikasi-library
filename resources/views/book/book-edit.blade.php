...
<body class="bg-gray-100 text-gray-800">
    <div class="container mx-auto p-4">
        ...
        <form action="{{ route('books.update', $book->id) }}" method="POST" class="bg-white p-6 rounded-lg shadow-lg max-w-lg mx-auto">
            @csrf
            @method('PUT')
            <!-- Title -->
            <div class="mb-4">
                ...
                <div class="flex flex-wrap">
                    @foreach ($categories as $category)
                        <div class="mr-4 mb-2">
                            ...
                            >
                            <label for="category_{{ $category->id }}" class="text-gray-600">{{ $category->category_name }}</label>
                        </div>
                    @endforeach
                </div>
            </div>
